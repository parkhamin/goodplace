...
<script>
		...
		function modify( f ){
			...
			//let admin = f.userpower.value; // 관리자
			...
			if (session.getAttribute("admin") != null){
				...
				if( pwd == c_pwd ){ // 관리자가 아닐때 비밀번호 일치 확인
					f.action = 'board_update_form.do';
					f.submit();//컨트롤러로 pwd, idx, name,content정보를 넘긴다
				}
				else{
		...
		function del( f ){
			...
			if(session.getAttribute("admin") != null){ // 관리자 일때
	...
	<div id="main_box">
		...
		<c:forEach var="vo" items="${ list }">
			...
			<div class="visit_box">
				...
				<form>
					<input type="hidden" name="pwd" value="${ vo.pwd }"> <input
						...
						<!-- <input type="hidden" name="userpower" value="${ vo.userpower }"> -->

					비밀번호<input type="password" name="c_pwd"> <input
						type="button" value="수정" onclick="modify(this.form);"> <input
